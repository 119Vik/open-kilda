...
public class FlowController {
     ...
     *
     * @param correlationId correlation ID header value.
     * @return a detailed response of the invalidate operation (added, deleted, modified, unchanged flows)
     */
    @ApiOperation(value = "Invalidate Flow Cache(s)", response = FlowCacheSyncResults.class)
    @ApiResponses(value = {
            @ApiResponse(code = 200, response = FlowCacheSyncResults.class, message = "Operation is successful")})
    @RequestMapping(path = "/flows/cache/invalidate",
            method = RequestMethod.PUT)
    @ResponseStatus(HttpStatus.OK)
    ...
    @ApiResponses(value = {
            @ApiResponse(code = 200, response = FlowCacheSyncResults.class, message = "Operation is successful")})
    @RequestMapping(path = "/flows/cache/refresh",
            method = RequestMethod.PUT)
    @ResponseStatus(HttpStatus.OK)
